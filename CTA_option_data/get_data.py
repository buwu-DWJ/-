    ...
    def get_fadj_data_given_hotmonth(self):
        for und in self.und_list:
            ...
            df = pd.read_excel(os.path.join(
                thisDir, und.split('.')[1]+'_close.xlsx'), index_col=0)
...
# percentile_list = [10, 25, 50, 75, 90]
# fig = plt.figure(
#     figsize=(8, 3*len(my_cdp.und_list)))
# for i, und in enumerate(my_cdp.und_list):
#     tau_hotmonth = my_cdp.tau[und+'_hot'][TODAY_STR]
#     tau_subhotmonth = my_cdp.tau[und+'_subhot'][TODAY_STR]
#     len_fig = max(tau_hotmonth, tau_subhotmonth)+2
#     iv_hotmonth = my_cdp.iv[und+'_hot']
#     iv_subhotmonth = my_cdp.iv[und+'_subhot']
#     ax = fig.add_subplot(len(my_cdp.und_list), 1, i+1)
#     for vol_type in ['yz']:
#         df = my_cdp.vol_df[und+'_'+vol_type]
#         for percentile in percentile_list:
#             locals()[f'pct_{percentile}'] = []
#         for i in range(len_fig):
#             csd_array = df[i+1].values[i+1:]
...
#                       f'pct_{percentile}'], label=str(percentile))
#         if iv_hotmonth:
#             percentile_hotmonth = percentileofscore(df[tau_hotmonth].dropna().values, iv_hotmonth)
#             plt.annotate(f'{iv_hotmonth}({percentile_hotmonth:.0f})', xy=(tau_hotmonth, iv_hotmonth), xytext=(tau_hotmonth, iv_hotmonth+2))
#             plt.scatter([tau_hotmonth],[iv_hotmonth])
#         if iv_subhotmonth:
#             percentile_subhotmonth = percentileofscore( df[tau_subhotmonth].dropna().values, iv_subhotmonth)
#             plt.annotate(f'{iv_subhotmonth}({percentile_subhotmonth:.0f})', xy=(tau_subhotmonth, iv_subhotmonth), xytext=(tau_subhotmonth, iv_subhotmonth+2))
#             plt.scatter([tau_subhotmonth],[iv_subhotmonth])
#         plt.title(und+'_'+vol_type, fontsize=15)
#         plt.legend()
#         plt.grid()
...
#         plt.yticks(fontsize=12)
        ...
        # for i, csd_und in enumerate(und_list):
        #     hot_month = pd.read_excel(
        #         os.path.join(thisDir, 'data.xlsx'), sheet_name='hotmonth_'+csd_und.split('.')[1], index_col=0)
        #     print('当前开始获取期权原始dogsk数据')
        #     for j, csd_date in enumerate(trading_day[:END_IDX]):
        #         if j == 0:
        #             continue
        #         try:
        ...
        #             for temp_ in symbol_history['Node']:
        #                 if temp_['ENG'].split('(')[0] == csd_und.split('.')[0]:
        #                     for temp__ in temp_['Node']:
        #                         if temp__['Node'][0]['Node'][0]['Contracts'][0].split('.')[3] == csd_und.split('.')[1]:
        #                             for temp___ in temp__['Node']:
