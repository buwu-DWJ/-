# -*- coding: utf-8 -*-
"""
Created on Wed Dec 14 15:41:59 2022

@author: mrlia
"""

import pandas as pd
import symbols159901 as sb
import datetime
...
#    mpercent = mprofit / average
    ...
    dayreturn = pd.read_excel('深圳100ETF日内损益.xlsx')[['日期', name]]
    month_r = []
    year_r = []
    ...
    dayfee = pd.read_excel('深圳100ETF手续费.xlsx')[['日期',name]]
    month_f = []
    year_f = []
    ...
    return ([position_all, position_day, cashgreeks, return_frame, head, tcashgreeks,ymerged3,ymerged2,y_vmerged,tmerged,strike,y_vmerged_permon,tmerged_permon,tfee_permon, ymerged_permon,ymerged])
ls7 = GenerateFrame('LS7')


permonls7 = pd.concat([ls7[11],ls7[12],ls7[13],ls7[14]], axis = 1)
permonls7['account'] = 'LS7'


pls7 = pd.merge(ls7[11],ls7[12], on = 'EXE_ENDDATE', how = 'outer')
pls7 = pd.merge(pls7,ls7[13], on = 'EXE_ENDDATE', how = 'outer')
pls7 = pd.merge(pls7,ls7[14], on = 'EXE_ENDDATE', how = 'outer')
pls7.columns = ['月份', '隔日', '日内', '手续费', '昨持']
pls7.to_excel('深圳100ETFLS7dailyreturn.xlsx')
