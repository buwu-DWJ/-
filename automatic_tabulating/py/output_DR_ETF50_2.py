...
''''''''''''''''''''''''''''''
    dfy1 = pd.read_excel('德睿每日损益' + sb.yesterday + '.xlsx', sheetname = None)
    etfdata = sb.etf[['Pre_close', 'RT_LATEST']]
    etfchg = float((etfdata['RT_LATEST'] - etfdata['Pre_close']) / etfdata['Pre_close'])
    if underlying == '50ETF':
        name_ud = 'ETF50'
    else:
        name_ud = 'SH300'
    
    dt = dfy1[name_ud + name + 'Greeks']
    
    delta_afct = dt['Unnamed: 1'][13] * etfchg
''''''''''''''''''''''''
...
#    mpercent = mprofit / average
    ...
    return([position_all, position_day, cashgreeks, return_frame, head, tcashgreeks,ymerged3,ymerged2,y_vmerged,tmerged,strike,y_vmerged_permon,tmerged_permon,tfee_permon, ymerged_permon, df5, df2])#####################
...
#permondr = pd.concat([dr[11],dr[12],dr[13],dr[14]], axis = 1)
#permondr['account'] = 'DR'
permondb = pd.concat([db[11],db[12],db[13],db[14]], axis = 1)
permondb['account'] = 'DB'
...
permonls['account'] = 'LS'
'''
pdr = pd.merge(dr[11],dr[12], on = 'EXE_ENDDATE', how = 'outer')
pdr = pd.merge(pdr,dr[13], on = 'EXE_ENDDATE', how = 'outer')
pdr = pd.merge(pdr,dr[14], on = 'EXE_ENDDATE', how = 'outer')
pdr.columns = ['月份', '隔日', '日内', '手续费', '昨持']
pdr.to_excel('50DRdailyreturn.xlsx')

'''
pls = pd.merge(ls[11],ls[12], on = 'EXE_ENDDATE', how = 'outer')
pls = pd.merge(pls,ls[13], on = 'EXE_ENDDATE', how = 'outer')
pls = pd.merge(pls,ls[14], on = 'EXE_ENDDATE', how = 'outer')
pls.columns = ['月份', '隔日', '日内', '手续费', '昨持']
pls.to_excel('50dailyreturn.xlsx')
#permon = pd.concat([permondr, permondb, permonyx, permonls])
permon = pd.concat([permondb, permonls])
