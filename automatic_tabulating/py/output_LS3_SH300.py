...
#    mpercent = mprofit / average
    ...
    dayreturn = pd.read_excel('300SH日内损益.xlsx')[['日期', name]]
    month_r = []
    year_r = []
    ...
    dayfee = pd.read_excel('300SH手续费.xlsx')[['日期',name]]
    month_f = []
    year_f = []
...
permonls3 = pd.concat([ls3[10],ls3[11],ls3[12],ls3[13]], axis = 1)
permonls3['account'] = 'LS3'
permonls4 = pd.concat([ls4[10],ls4[11],ls4[12],ls4[13]], axis = 1)
...
pls3 = pd.merge(ls3[10],ls3[11], on = 'EXE_ENDDATE', how = 'outer')
pls3 = pd.merge(pls3,ls3[12], on = 'EXE_ENDDATE', how = 'outer')
pls3 = pd.merge(pls3,ls3[13], on = 'EXE_ENDDATE', how = 'outer')
pls3.columns = ['月份', '隔日', '日内', '手续费', '昨持']
pls3.to_excel('300SHLS3dailyreturn.xlsx')

pls4 = pd.merge(ls4[10],ls4[11], on = 'EXE_ENDDATE', how = 'outer')
pls4 = pd.merge(pls4,ls4[12], on = 'EXE_ENDDATE', how = 'outer')
pls4 = pd.merge(pls4,ls4[13], on = 'EXE_ENDDATE', how = 'outer')
pls4.columns = ['月份', '隔日', '日内', '手续费', '昨持']
pls4.to_excel('300SHLS4dailyreturn.xlsx')
...
worksheet10.write('A19', '300ETF', format4)
