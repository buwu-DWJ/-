...
from WindPy import *
w.start()
contracts = w.wset("optionchain","us_code=510500.SH;option_var=全部;call_put=全部").Data[3]
...
def LoadHV(interval=130):
    error,data = w.wsd("510500.SH", "close,open,high,low", w.tdaysoffset(-interval).Data[0][0], w.tdaysoffset(-1).Data[0][0],  usedf=True)
    return(data)
...
error,etf = w.wsq("510500.SH", "rt_open,rt_high,rt_low,rt_bid1,rt_ask1,rt_latest", usedf=True)
#preclose = w.wss("510300.SH", "pre_close","tradeDate="+today+";priceAdj=U;cycle=D").Data[0]
error,pre_etf = w.wsq("510500.SH", "rt_pre_close", usedf=True)
