...
#    mprofit = sum( df_m['盈亏'] )
    ...
    for i in range(lenth+1):
        year2.append( str(df5['年份'].iloc[i]) )
    df5['month'] = month
    df5['year'] = year2
    tmp_m = sb.today[4:6]
    tmp_y = sb.today[0:4]
...
#    mpercent = mprofit / average
    ...
    dayreturn = pd.read_excel('创业板ETF日内损益.xlsx')[['日期', name]]
    month_r = []
    year_r = []
    ...
    dayfee = pd.read_excel('创业板ETF手续费.xlsx')[['日期',name]]
    month_f = []
    year_f = []
    ...
    return([position_all, position_day, cashgreeks, return_frame, head, tcashgreeks,ymerged3,ymerged2,y_vmerged,tmerged,strike,y_vmerged_permon,tmerged_permon,tfee_permon, ymerged_permon])

ls3 = GenerateFrame('LS3')
ls4 = GenerateFrame('LS4')
#yx = GenerateFrame('YX')
#ls = GenerateFrame('LS')


permonls3 = pd.concat([ls3[11],ls3[12],ls3[13],ls3[14]], axis = 1)
permonls3['account'] = 'LS3'
permonls4 = pd.concat([ls4[11],ls4[12],ls4[13],ls4[14]], axis = 1)
permonls4['account'] = 'LS4'
#permonyx = pd.concat([yx[11],yx[12],yx[13],yx[14]], axis = 1)
#permonyx['account'] = 'YX'
#permonls = pd.concat([ls[11],ls[12],ls[13],ls[14]], axis = 1)
#permonls['account'] = 'LS'

pls3 = pd.merge(ls3[11],ls3[12], on = 'EXE_ENDDATE', how = 'outer')
pls3 = pd.merge(pls3,ls3[13], on = 'EXE_ENDDATE', how = 'outer')
pls3 = pd.merge(pls3,ls3[14], on = 'EXE_ENDDATE', how = 'outer')
pls3.columns = ['月份', '隔日', '日内', '手续费', '昨持']
pls3.to_excel('创业板ETFLS3dailyreturn.xlsx')

pls4 = pd.merge(ls4[11],ls4[12], on = 'EXE_ENDDATE', how = 'outer')
pls4 = pd.merge(pls4,ls4[13], on = 'EXE_ENDDATE', how = 'outer')
pls4 = pd.merge(pls4,ls4[14], on = 'EXE_ENDDATE', how = 'outer')
pls4.columns = ['月份', '隔日', '日内', '手续费', '昨持']
pls4.to_excel('创业板ETFLS4dailyreturn.xlsx')

permon = pd.concat([permonls3, permonls4])
#permon = pd.concat([permondr, permondb, permonyx, permonls])
...
writer = pd.ExcelWriter(underlying + '聊塑3、4每日损益'+today+'.xlsx')
...
ls3[2].to_excel(writer, sheet_name='LS3Greeks', index=False, startrow=2, header=False, float_format='%.0f')
ls3[5].to_excel(writer, sheet_name='LS3Greeks', index=False, startrow=10, header=False, float_format='%.0f')
ls3[3].to_excel(writer, sheet_name='LS3Greeks', index=False, startrow=17, header=False, float_format='%.0f')
ls3[0].to_excel(writer, sheet_name='LS3持仓_当日交易', index=False, startrow=2, header=False)
ls3[1].to_excel(writer, sheet_name='LS3持仓_当日交易', index=False, startrow=16, header=False)

ls4[2].to_excel(writer, sheet_name='LS4Greeks', index=False, startrow=2, header=False, float_format='%.0f')
ls4[5].to_excel(writer, sheet_name='LS4Greeks', index=False, startrow=10, header=False, float_format='%.0f')
ls4[3].to_excel(writer, sheet_name='LS4Greeks', index=False, startrow=17, header=False, float_format='%.0f')
ls4[0].to_excel(writer, sheet_name='LS4持仓_当日交易', index=False, startrow=2, header=False)
ls4[1].to_excel(writer, sheet_name='LS4持仓_当日交易', index=False, startrow=16, header=False)
...
worksheet1 = worksheets['LS3持仓_当日交易']
worksheet3 = worksheets['LS3Greeks']
...
worksheet4 = worksheets['LS4持仓_当日交易']
worksheet6 = worksheets['LS4Greeks']
