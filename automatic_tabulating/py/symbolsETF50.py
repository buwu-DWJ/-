...
from WindPy import *
w.start()
contracts = w.wset("optionchain","us_code=510050.SH;option_var=全部;call_put=全部").Data[3]
#tmp=w.wset("optioncontractbasicinfo","exchange=sse;windcode=510050.SH;status=trading;field=wind_code").Data[0]
#for i in range(len(contracts)):
#    contracts[i] = contracts[i] + '.SH'
...
def LoadData(symbols):
    ...
    #data['50ETF'] = w.wsq('510050.SH','rt_bid1').Data[0][0]
...
def LoadHV(interval=130):
    error,data = w.wsd("510050.SH", "close,open,high,low", w.tdaysoffset(-interval).Data[0][0], w.tdaysoffset(-1).Data[0][0],  usedf=True) 
    return(data)
...
def LoadVolume(name):
    ...
    df2['Contract'] = df2['合约代码'].apply(str) + '.SH'
...
error,etf = w.wsq("510050.SH", "rt_open,rt_high,rt_low,rt_bid1,rt_ask1,rt_latest", usedf=True)
#preclose = w.wss("510050.SH", "pre_close","tradeDate="+today+";priceAdj=U;cycle=D").Data[0]
error,pre_etf = w.wsq("510050.SH", "rt_pre_close", usedf=True)
